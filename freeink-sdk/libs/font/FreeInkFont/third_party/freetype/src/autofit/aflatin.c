...
#define FLAT_THRESHOLD( x )  ( x / 14 )
                                ...
                                FT_Face          face )
  {
...

    {
      ...
      int                 dim;
      AF_LatinMetrics     dummy = NULL;
      AF_Scaler           scaler = NULL;
      ...
      /* FreeInk: keep large one-time auto-hint records in the caller's
       * bounded workspace instead of nesting them on the render stack. */
      if ( FT_NEW( hints ) )
        goto Exit;
      af_glyph_hints_init( hints, memory );
      if ( FT_NEW( dummy ) )
        goto Exit;
      scaler = &dummy->root.scaler;
